...
module.exports = async function(message, member, durationobject, reason){
	...
	try {
		...
		if (res.value.count < 7 && durationobject !== 'perma') {
			...
			exec(`schtasks /Create /TN bansreset\\${memberid} /TR "node ${banresetpath} ${memberid}" /SD ${date.plus({days: config.banreset[res.value.count]}).toFormat('LL\'/\'dd\'/\'yyyy')} /ST ${date.plus({days: config.banreset[res.value.count]}).toFormat('HH\':\'mm')} /SC ONCE /F`);
		} else {
