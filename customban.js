...
module.exports = async function (message, member, durationobject, reason) {
	...
	let date = DateTime.local().setZone("America/Buenos_Aires").startOf("minute");
	...
	try {
		...
		if (res.value.count < 7 && durationobject !== "perma") {
			exec(
				`schtasks /Create /TN mmbans\\${memberid} /TR "node ${unbanpath} ${memberid}" /SD ${date
					.plus(durationobject)
					.toFormat("LL'/'dd'/'yyyy")} /ST ${date
					.plus(durationobject)
					.toFormat("HH':'mm")} /SC ONCE /F`
			);
			exec(
				`schtasks /Create /TN bansreset\\${memberid} /TR "node ${banresetpath} ${memberid}" /SD ${date
					.plus({ days: config.banreset[res.value.count] })
					.toFormat("LL'/'dd'/'yyyy")} /ST ${date
					.plus({ days: config.banreset[res.value.count] })
					.toFormat("HH':'mm")} /SC ONCE /F`
			);
		...
		await db.collection("banlog").insertOne({
			...
			enddate:
				durationobject === "perma" ? null : date.plus(durationobject).toISO(),
			reason: reason
		});
		const banrembed = new Discord.RichEmbed()
			.setTitle(
				`Usuario baneado ${
					durationobject === "perma"
						? "**indefinidamente**"
						: `por ${calculatetime(durationobject)}`
				}`
			)
			.setColor("RED")
			...
			.addField(
				...
				durationobject === "perma"
					? "Indefinida"
					: date.plus(durationobject).toFormat("dd'/'LL'/'yyyy HH':'mm")
			)
			.addField("Motivo de ban:", reason)
			.addField("Nivel de ban actual:", res.value.count)
			.setFooter(
				"Baneado por: " + message.member.displayName,
