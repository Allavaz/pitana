...
async function fixBanResets() {
	...
	try {
		...
		let items = await db.collection('banlist').find({count: {$gt: 0}}).toArray();
		let tofix = [];
		for (let i=0; i<items.length; i++) {
			if (DateTime.fromISO(items[i].lastban).plus({days: config.banreset[items[i].count]}).toISO() < now) {
				tofix.push(items[i]._id);
			}
		}
		let res = await db.collection('banlist').updateMany({_id: {$in: tofix}}, {$set: {count: 0}});
