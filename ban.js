...
module.exports = async function(message, member, reason){
	...
	try {
		...
		if (res.value.count < 7) {
			...
			exec(`schtasks /Create /TN bansreset\\${memberid} /TR "node ${banresetpath} ${memberid}" /SD ${date.plus(config.banreset).toFormat('LL\'/\'dd\'/\'yyyy')} /ST ${date.plus(config.banreset).toFormat('HH\':\'mm')} /SC ONCE /F`);
		} else {
