...
module.exports = async function (message, member) {
	...
	try {
		...
		if (banlistitem.isBanned) {
			let banlogitem = await db
				.collection("banlog")
				.findOne({ playerid: memberid }, { sort: { startdate: -1 } });
			baninforembed
				.addField(
					"Fecha de inicio:",
					DateTime.fromISO(banlogitem.startdate).toFormat(
						"dd'/'LL'/'yyyy HH':'mm"
					)
				)
			...
			if (banlogitem.enddate !== null) {
				...
				const initialduration = Interval.fromDateTimes(start, end)
					.toDuration(["days", "hours", "minutes"])
					.toObject();
				const remainingduration = Interval.fromDateTimes(now, end)
					.toDuration(["days", "hours", "minutes"])
					.toObject();
				baninforembed
					.setDescription(
						`${member} se encuentra **baneado** por **${calculatetime(
							initialduration
						)}** del matchmaking.`
					)
					.addField(
						"Fecha de expiración:",
						DateTime.fromISO(banlogitem.enddate).toFormat(
							"dd'/'LL'/'yyyy HH':'mm"
						)
					)
					.addField(
						"Tiempo restante del ban:",
						calculatetime(remainingduration)
					);
			} else {
				baninforembed.setDescription(
					`${member} se encuentra **baneado indefinidamente** del matchmaking.`
				);
			}
			...
			baninforembed
				.setDescription(
					`${member} **no** se encuentra baneado del matchmaking.`
				)
				.addField("Nivel de ban actual:", banlistitem.count);
		}
		if (banlistitem.count > 0) {
			const banresetdate = DateTime.fromISO(banlistitem.lastban)
				.plus({ days: config.banreset[banlistitem.count] })
				.toFormat("dd'/'LL'/'yyyy HH':'mm");
			baninforembed.addField("Fecha de reseteo de nivel de ban:", banresetdate);
			const remainingbanreset = Interval.fromDateTimes(
				now,
				DateTime.fromISO(banlistitem.lastban).plus({
					days: config.banreset[banlistitem.count]
				})
			)
				.toDuration(["days", "hours", "minutes"])
				.toObject();
			baninforembed.addField(
				"Tiempo restante para el reseteo de nivel de ban:",
				calculatetime(remainingbanreset)
			);
		}
		msg.delete();
